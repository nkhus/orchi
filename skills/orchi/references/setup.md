...
Follow the bundled [operator guide](operator-guide.md). Key generation and signing use `<skills>/orchi/scripts/orchi_operator.py`, not a tool from an external source checkout. Only the public key belongs in policy. Never create or use a private signing key on the human's behalf inside a worker session.
